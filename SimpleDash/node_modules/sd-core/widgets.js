module.exports = {
    chart: function() {
        return new SimpleDashChartWidget();
    },
    table: function() {
        return new SimpleDashTableWidget();
    }
};
...
function SimpleDashChartWidget() {
    ...
    this.populate = function (elem, config, data) {
        ...
        if (data.type == "chart") {
            ...
            throw new Error("Unsupported data source: " + data.type);
        }
    }
}

/**
 * Table widget based on jTable.
 * @class SimpleDashTableWidget
 */
function SimpleDashTableWidget() {
    ...
    this.populate = function (elem, config, data) {
        for (var field in config["fields"]) {
            if (config["fields"][field]["sd-display"]) {
                ...
                config["fields"][field]["display"] = function (data) {
                    // XXX: looks like a hack. Need to change this to call to a node.js module
                    return window.Handlebars.compile(template)({data: data});
                }
            }
        }
        ...
        for (var i = 0; i < rows.length; i++) {
            elem.jtable('addRecord', {record: rows[i], clientOnly: true });
        }
